Module modARInvoice_Cardcode
    ...
    Public Function ProcessARInvoice_CardCode(ByVal oDv As DataView, ByVal file As System.IO.FileInfo, ByRef sErrDesc As String) As Long
        ...
        Try
            ...
            odtDatatable.Columns.Add("TPAClinicCode", GetType(String))
            ...
            For intRow As Integer = 0 To odtDatatable.Rows.Count - 1
                If Not (odtDatatable.Rows(intRow).Item(1).ToString.Trim() = String.Empty Or odtDatatable.Rows(intRow).Item(1).ToString.ToUpper().Trim() = "COMPANY_CODE") Then
                    ...
                    sSQL = "SELECT COUNT(""U_cln_code"") AS ""MNO"" FROM " & p_oCompDef.sSAPDBName & ".""@AE_TPA_APCODE"" WHERE UPPER(""U_cln_code"") = '" & sClinicCode.ToUpper() & "'"
                    If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Executing SQL " & sSQL, sFuncName)
                    ...
                    If iCount > 0 Then
                        odtDatatable.Rows(intRow)("TPAClinicCode") = "TPA"
                    Else
                        odtDatatable.Rows(intRow)("TPAClinicCode") = "NONTPA"
                    End If
                End If
            Next
            ...
            If p_oCompany.Connected Then
                ...
                If oDvFinalView.Count > 0 Then
                    ...
                    If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Group by TPA Listing table", sFuncName)
                    Dim oDtGroup As DataTable = oDvFinalView.Table.DefaultView.ToTable(True, "TPAClinicCode")
                    For i As Integer = 0 To oDtGroup.Rows.Count - 1
                        If Not (oDtGroup.Rows(i).Item(0).ToString.Trim() = String.Empty Or oDtGroup.Rows(i).Item(0).ToString.ToUpper().Trim() = "TPACLINICCODE") Then
                            Dim sTPAClinicType As String = oDtGroup.Rows(i).Item(0).ToString.Trim()

                            oDvFinalView.RowFilter = "TPAClinicCode = '" & oDtGroup.Rows(i).Item(0).ToString.Trim() & "' "
                            If oDvFinalView.Count > 0 Then
                                If sTPAClinicType = "TPA" Then
                                ...
                                ElseIf sTPAClinicType = "NONTPA" Then
                                    If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Calling ProcessDatas_NonTPAListings()", sFuncName)
    ...
    Public Function ProcessARInvoice_CardCode_Working(ByVal oDv As DataView, ByVal file As System.IO.FileInfo, ByRef sErrDesc As String) As Long
        Dim sFuncName As String = "ProcessARInvoice_CardCode"
        Dim sSQL As String
        ...
        Try
            ...
            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Completed with SUCCESS", sFuncName)
            ProcessARInvoice_CardCode_Working = RTN_SUCCESS
        Catch ex As Exception
            sErrDesc = ex.Message
            Call WriteToLogFile(sErrDesc, sFuncName)
            ...
            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Completed with ERROR", sFuncName)
            ProcessARInvoice_CardCode_Working = RTN_ERROR
        End Try
    End Function
